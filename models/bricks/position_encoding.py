...
def get_sine_pos_embed_morph(coords, embed_dim, temperature=10000.0, exchange_xy=True):
    """
    安全的位置编码生成函数
    Args:
        coords: [..., 2] 归一化到[0,1]的坐标
        embed_dim: 输出维度(必须为偶数)
    """
    # 输入验证
    assert embed_dim % 2 == 0, f"embed_dim必须是偶数，当前为{embed_dim}"
    # 自动归一化（如果尚未归一化）
    if not (torch.all(coords >= 0) and torch.all(coords <= 1)):
        coords = coords - coords.amin(dim=-2, keepdim=True)
        coords = coords / (coords.amax(dim=-2, keepdim=True) + 1e-6)
    ...
    # 生成位置编码
    dim_t = torch.arange(embed_dim // 2, dtype=torch.float32, device=coords.device)
    dim_t = temperature ** (2 * (dim_t // 2) / (embed_dim // 2))
    
    pos = coords[..., None] * dim_t  # [..., 2, dim/2]
    pos = torch.stack([pos[..., 0].sin(), pos[..., 0].cos(),
                      pos[..., 1].sin(), pos[..., 1].cos()], dim=-1)
    
    # 安全的xy交换
    if exchange_xy and pos.shape[-2] >= 2:
        pos[..., :2, :] = pos[..., [1, 0], :]  # 直接交换前两个位置
